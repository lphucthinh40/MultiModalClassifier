...
def createCNNsimplemodel(name, numclasses, img_shape, metrics=['accuracy']):
    ...
    elif name=='xceptionmodel2':
        return create_Xceptionmodel2(numclasses, img_shape, metrics)
    elif name=='efficientnetB1':
        return create_efficientnetB1model(numclasses, img_shape, metrics)
    elif name=='efficientnetB4':
        return create_efficientnetB4model(numclasses, img_shape, metrics)
...
#add data augmentation into the model
def create_efficientnetB1model(numclasses, img_shape, metrics=['accuracy']):
    data_augmentation = tf.keras.Sequential([
        tf.keras.layers.experimental.preprocessing.RandomTranslation(0.1,0.1),
        tf.keras.layers.experimental.preprocessing.RandomRotation(0.1),
        tf.keras.layers.experimental.preprocessing.RandomZoom(0.15),        
...
#add data augmentation into the model
def create_efficientnetB4model(numclasses, img_shape, metrics=['accuracy']):
    data_augmentation = tf.keras.Sequential([
        tf.keras.layers.experimental.preprocessing.RandomTranslation(0.1,0.1),
        tf.keras.layers.experimental.preprocessing.RandomRotation(0.1),
        tf.keras.layers.experimental.preprocessing.RandomZoom(0.15),        
...
def create_Xceptionmodel2(numclasses, img_shape, metrics=['accuracy']):
    pretrained_model = tf.keras.applications.Xception(input_shape=img_shape, include_top=False, weights='imagenet')#Downloading data from https://storage.googleapis.com/tensorflow/keras-applications/xception/xception_weights_tf_dim_ordering_tf_kernels_notop.h5
    pretrained_model.trainable = True #False #True
    data_augmentation = tf.keras.Sequential([
        # tf.keras.layers.experimental.preprocessing.RandomFlip('horizontal'),
